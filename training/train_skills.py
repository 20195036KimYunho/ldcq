...
if env_name == 'ARC':
    ...
    dataset = get_dataset(env_name, H, stride, test_split, get_rewards=args.get_rewards, separate_test_trajectories=args.separate_test_trajectories, append_goals=args.append_goals, all_task=False, tasks=tasks)

    obs_chunks_train = dataset['observations_train']
    opr_chunks_train = dataset['operations_train']
    sel_chunks_train = dataset['selections_train']

    # train chunk의 형태
    print("Train_Observations:", obs_chunks_train.shape)  # chunk size x T(H) x s_dim
    print("Train_Operations:", opr_chunks_train.shape)
    print("Train_Selections:", sel_chunks_train.shape)

    if test_split > 0.0:
        obs_chunks_test = dataset['observations_test']
        # action chunks_test
        opr_chunks_test = dataset['operations_test']
        sel_chunks_test = dataset['selections_test']

    _, _, state_dim = obs_chunks_train.shape
    _, _, opr_dim = opr_chunks_train.shape
    _, _, sel_dim = opr_chunks_train.shape
    a_dim = opr_dim+sel_dim
    ...
    # train chunk의 형태
    print("Train_Observations:", obs_chunks_train.shape)  # chunk size x T(H) x s_dim
    print("Train_actions:", action_chunks_train.shape)

    # for key, value in dataset.items():
    #     print(f"The first value of key '{key}' is: {value[0]}")
...
model = SkillModel(state_dim, a_dim, z_dim, h_dim, horizon=H, a_dist=a_dist, beta=beta, fixed_sig=None, encoder_type=encoder_type, state_decoder_type=state_decoder_type, policy_decoder_type=policy_decoder_type,
                   per_element_sigma=per_element_sigma, conditional_prior=conditional_prior, train_diffusion_prior=train_diffusion_prior, normalize_latent=args.normalize_latent).cuda()
