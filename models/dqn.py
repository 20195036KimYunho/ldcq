...
class DDQN(nn.Module):
    ...
    def learn(self, env_name, dataload_train, dataload_test=None, n_epochs=10000, update_frequency=1, diffusion_model_name='', cfg_weight=0.0, per_buffer=0.0, batch_size=128, q_checkpoint_dir=''):
        # assert self.diffusion_prior is not None
        wandb.init(project=env_name,
                   name='train_qnet/'+diffusion_model_name+'{}.pt'.format('_PERbuffer' if per_buffer == 1 else ''),
                   config={
        ...
        for ep in tqdm(range(n_epochs), desc="Epoch"):
            ...
            if per_buffer:
                ...
                pbar = range(len(dataload_train) // batch_size)
                for _ in pbar:  # same num_iters as w/o PER
                    ...
                    if steps_total % update_frequency == 0:
                        ...
                        if steps_total % (update_steps) == 0:
                            wandb.log({"train_qnet/train_loss_0": loss_net_0},steps_total//3000)
                            wandb.log({"train_qnet/train_loss_1": loss_net_1},steps_total//3000)
                            wandb.log({"train_qnet/train_loss": loss_total},step=steps_total//3000)
                    ...
                    if steps_total % (update_steps) == 0:
                        torch.save(self,  q_checkpoint_dir+'/'+diffusion_model_name+'_dqn_agent_'+str(steps_total//update_steps)+'_cfg_weight_'+str(cfg_weight)+'{}.pt'.format('_PERbuffer' if per_buffer == 1 else ''))
                ...
                for s0, z, sT, reward in pbar:
                    ...
                    if steps_total % update_frequency == 0:
                        ...
                        if steps_total % (3000) == 0:
                            wandb.log({"train_qnet/train_loss_0": loss_net_0},step=steps_total//3000)
                            wandb.log({"train_qnet/train_loss_1": loss_net_1},step=steps_total//3000)
                            wandb.log({"train_qnet/train_loss": loss_total},step=steps_total//3000)
                        # wandb.log({"train_qnet/train_loss_0": loss_net_0})
                        # wandb.log({"train_qnet/train_loss_1": loss_net_1})
                        # wandb.log({"train_qnet/train_loss": loss_total})
                        loss_net_0, loss_net_1, loss_total = 0, 0, 0
                        steps_net_0, steps_net_1 = 0, 0
                        # self.target_net_0 = copy.deepcopy(self.q_net_0)
                        # self.target_net_1 = copy.deepcopy(self.q_net_1)
                    ...
                    if steps_total % (3000) == 0:
                        torch.save(self,  q_checkpoint_dir+'/'+diffusion_model_name+'_dqn_agent_'+str(steps_total//3000)+'_cfg_weight_'+str(cfg_weight)+'{}.pt'.format('_PERbuffer' if per_buffer == 1 else ''))
                        beta = np.min((beta+0.03, 1))

            self.scheduler_0.step()
            self.scheduler_1.step()
            wandb.log({"train_qnet/train_loss_episode": loss_ep/n_batch})
            epoch += 1