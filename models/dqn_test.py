...
class DDQN(nn.Module):
    ...
    def learn(self, dataload_train, dataload_test=None, n_epochs=10000, update_frequency=1, diffusion_model_name='', cfg_weight=0.0, per_buffer = 0.0, batch_size = 128):
        ...
        wandb.init(project="D4RL",
                name='train_q_net/'+diffusion_model_name,
                config={
            ...
            if per_buffer:
                ...
                for _ in pbar: # same num_iters as w/o PER
                    ...
                    if steps_total%update_frequency == 0:
                        ...
                        wandb.log({"train_q_net/train_loss_0": loss_net_0})
                        wandb.log({"train_q_net/train_loss_1": loss_net_1})
                        wandb.log({"train_q_net/train_loss": loss_total})
                    ...
                    if steps_total%(update_steps) == 0:
                        torch.save(self,  parent_folder+'/q_checkpoints_test/'+diffusion_model_name+'_dqn_agent_'+str(steps_total//update_steps)+'_cfg_weight_'+str(cfg_weight)+'{}.pt'.format('_PERbuffer' if per_buffer == 1 else ''))
            else:
                ...
                for s0,z,sT,reward in pbar:
                    ...
                    if steps_total%(3000) == 0:
                        torch.save(self,  parent_folder+'/q_checkpoints_test/'+diffusion_model_name+'_dqn_agent_'+str(steps_total//5000)+'_cfg_weight_'+str(cfg_weight)+'{}.pt'.format('_PERbuffer' if per_buffer == 1 else ''))

            beta = np.min((beta+0.01,1))
            self.scheduler_0.step()
            self.scheduler_1.step()
            wandb.log({"train_q_net/train_loss_episode": loss_ep/n_batch})
            epoch += 1
