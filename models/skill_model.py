...
class SkillModel(nn.Module):
    def __init__(self,state_dim,a_dim,z_dim,h_dim,horizon,a_dist='normal',beta=1.0,fixed_sig=None,encoder_type='gru',state_decoder_type='mlp',policy_decoder_type='mlp',
        ...
        if self.a_dist =='softmax' and self.use_contrastive:
            ...
            # self.contrastive_a = MarginCosineProduct(z_dim, 2*num_categocical_interval+1, self.a_dim).cuda()
        self.beta = beta
    ...
    def get_losses(self, states, actions, state_decoder):
        ...
        if self.decoder.ll_policy.a_dist == 'normal':
            ...
            actions_round = torch.round(actions*self.num_categocical_interval)/self.num_categocical_interval
            actions = ((actions_round+1)*self.num_categocical_interval).long() #Get class indices
        ...
        if self.decoder.ll_policy.a_dist == 'softmax' and self.use_contrastive:
            ...
            # z_tiled = z_sampled.tile([1, actions.shape[1]*self.a_dim, 1])
            # z_tiled = z_tiled.reshape(-1, self.a_dim, self.z_dim).contiguous()  # (batch * block_size, z_dim)
